...
void update_motcon(motiontype *p){
	...
	case mot_move:
		/// ------------------------------------
		clock_acceleration = max_acceleration * 0.010;
		
		if (fabs(p->speedcmd - currentspeed) > clock_acceleration){
			currentspeed += clock_acceleration;
		}else{
			currentspeed = p->speedcmd;
		}
	...
	case mot_turn:
		...
		if (fabs(currentspeed - p->speedcmd) > clock_acceleration){
			currentspeed += clock_acceleration;
		}else{
			currentspeed = p->speedcmd;
		}
...

		{
			...
			{ // Havent reach desired angle yet.
				p->motorspeed_r = (currentspeed)/2;
				//
				p->motorspeed_l = -(currentspeed)/2;
				//
			}
			else
			{// Reach desired angle.
